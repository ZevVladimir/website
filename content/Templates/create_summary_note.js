module.exports = async (params) => {
    ...
    const subproject = await quickAddApi.inputPrompt("Subproject name (e.g. NBODY):");
    ...
    const folder = await quickAddApi.suggester(folders, folders, "Which project folder?");
    if (!folder) return;

    const filename = `${subproject} Summary`;
    const filepath = `${folder}/${filename}.md`;

    if (app.vault.getAbstractFileByPath(filepath)) {
        new Notice(`⚠️ ${filepath} already exists.`);
        return;
    }
